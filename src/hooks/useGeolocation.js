...
export function useGeolocation() {
  ...
  const [autoCaptureStatus, setAutoCaptureStatus] = useState('idle'); // idle | searching | success | error | denied | timeout
  const [autoCaptureError, setAutoCaptureError] = useState(null);
  const [autoCaptureElapsed, setAutoCaptureElapsed] = useState(0);
  ...
  /**
   * Arrête le timer de capture automatique
   */
  const stopAutoCaptureTimer = useCallback(() => {
  ...
  /**
   * Lancement automatique d'une capture GPS (utilisé pendant l'enregistrement vocal)
   */
  const startAutoCapture = useCallback(
    async (options = {}) => {
      if (!isSupported) {
        ...
        setAutoCaptureStatus('error');
        setAutoCaptureError(notSupported);
        return { success: false, error: notSupported };
      }

      setAutoCaptureStatus('searching');
      setAutoCaptureElapsed(0);
      ...
      if (permissionState === 'denied') {
        const permissionError = {
          ...
          message: 'Permission géolocalisation refusée. Activez-la dans les réglages du navigateur.',
        };
        setAutoCaptureStatus('denied');
        setAutoCaptureError(permissionError);
        return { success: false, error: permissionError };
      }

      stopAutoCaptureTimer();
      autoCaptureTimerRef.current = setInterval(() => {
        setAutoCaptureElapsed((elapsed) => elapsed + 1);
      }, 1000);
      ...
      try {
        ...
        const pos = await getCurrentPosition(autoOptions);
        setAutoCaptureStatus('success');
        stopAutoCaptureTimer();
        setAutoCaptureElapsed(0);
        ...
        stopAutoCaptureTimer();
        const status =
          captureError?.code === 'PERMISSION_DENIED'
            ? 'denied'
            : captureError?.code === 'TIMEOUT'
            ? 'timeout'
            : 'error';
        setAutoCaptureStatus(status);
        setAutoCaptureError(captureError);
        return { success: false, error: captureError };
      }
    },
    [checkPermission, getCurrentPosition, isSupported, stopAutoCaptureTimer]
  );

  /**
   * Annule le processus automatique (ex: user annule avant fix GPS)
   */
