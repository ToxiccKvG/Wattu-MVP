 ...
 * 
 * Note : Ce layer ne fait PAS d'appels directs à Supabase
 * Il utilise les APIs (reportApi, storageApi)
 */

/**
 * Options de compression d'image par défaut
 */
const COMPRESSION_OPTIONS = {
  maxSizeMB: 1,              // Taille maximale : 1MB après compression
 ...
 * 
 * @param {File} file - Fichier image à comprimer
 ...
 * } else {
 *   console.log('Compression:', result.originalSize, '→', result.compressedSize);
...
export async function compressImage(file, options = {}) {
  try {
    ...
    // Validation : type d'image autorisé
    ...
    console.log(`🗜️ Compression en cours... (Taille originale: ${(originalSize / 1024).toFixed(2)}KB)`);

    // Compression avec options par défaut (ou override)
    ...
    console.log(`✅ Compression réussie: ${(originalSize / 1024).toFixed(2)}KB → ${(compressedSize / 1024).toFixed(2)}KB (${compressionRatio}% réduit)`);
...
/**
 * Valider les données du formulaire de signalement
 * 
...
export function validateReportData(formData) {
  ...
  // 3. Longitude obligatoire et valide (-180 à 180)
  ...
  // 4. Description optionnelle mais si fournie, max 1000 caractères
  if (formData.description && formData.description.length > 1000) {
    errors.description = 'Description trop longue (max 1000 caractères)';
  }

  // 5. Téléphone optionnel mais si fourni, format basique (+221...)
  ...
  // 6. Nom du citoyen optionnel mais si fourni, max 100 caractères
  if (formData.citizen_name && formData.citizen_name.length > 100) {
    errors.citizen_name = 'Nom trop long (max 100 caractères)';
  }
 ...
 * 2. Si image fournie : comprimer → uploader
 * 3. Créer le signalement avec l'URL de l'image
 * 4. Retourner le signalement créé
 * 
 * @param {Object} formData - Données du formulaire
...
export async function submitReport(formData) {
  try {
    console.log('📝 Soumission du signalement en cours...');
    ...
    if (formData.imageFile) {
      ...
      const uploadResult = await storageApi.uploadReportImage(
        ...
        null // reportId généré automatiquement (temp-{timestamp})
      );
    ...
    console.log('✅ Signalement créé avec succès:', reportResult.data.id);
    ...
    console.error('❌ Erreur inattendue submitReport:', err);
    return {
      report: null,
      error: {
...
export async function getAgentStatistics(communeId) {
  try {
    ...
    console.log(`📊 Calcul des statistiques pour la commune: ${communeId}`);

    // Récupérer TOUS les signalements de la commune (via RLS)
 ...
 * 
 * @param {Array} reports - Liste des signalements à exporter
 * @param {string} [filename='signalements.csv'] - Nom du fichier CSV
 * 
 * @returns {string} - Contenu CSV (à télécharger via un blob)
 * 
 * @example
...
export function exportToCSV(reports) {
  try {
    ...
    console.log(`📥 Export CSV de ${reports.length} signalements...`);
    ...
    const rows = reports.map(r => [
      r.id || '',
      r.type || '',
      (r.description || '').replace(/"/g, '""'), // Échapper les guillemets
      r.status || '',
      r.priority || '',
    ...
    const csvContent = [
      ...
      ...rows.map(row => row.map(cell => `"${cell}"`).join(',')) // Lignes de données
    ].join('\n');
...
/**
 * Télécharger un CSV automatiquement (helper function)
 * 
 ...
 * @returns analytics.top_communes - Top 5 communes classées
 * @returns analytics.by_type - Répartition par type de signalement
 * @returns analytics.trends - Évolution sur 30 derniers jours
 * 
 * @example
...
export async function calculateGlobalAnalytics() {
  try {
    console.log('🔄 Calcul analytics globales...');
    ...
    console.error('❌ Erreur calcul analytics:', err);
    return {
      analytics: null,
      error: {
